...
setMethod("getWindEstimate",signature = signature(groundSpeeds="matrix", phi="numeric",windStart="ANY"), function(groundSpeeds, phi, windStart) {
  ...
  if (optimResult$convergence == 0) {
    if (det(optimResult$hessian) != 0)
      covar = tryCatch(solve(optimResult$hessian / optimResult$value * n / 2),error=function(e) {covar=NULL})
    else
      covar = NULL
    list(
...
calcVar <- function(groundSpeeds, windSpeed, phi) {
  .Call('_moveWindSpeed_calcVar', PACKAGE = 'moveWindSpeed', groundSpeeds, windSpeed, phi)
}